...
trans=function(max,data,newname){
  newname=matrix(0,ncol=9,nrow=max)
  newname[,1]=data[,1]
  for (j in 1:max){
    for (i in 1:8){
  ...
  names(newname)=c("index1",paste("x",1:8,sep=""))
  return(newname)
}
#######將資料進行轉換
newtrain=trans(1934,train,newtrain)
newtest=trans(946,test,newtest)
####將0~9換成字符類型，否則後面會有問題
...
text(model)
####預測
predict=predict(model,newtest[,2:9])
